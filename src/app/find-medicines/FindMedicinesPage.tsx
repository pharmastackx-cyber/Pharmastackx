...
import UserDetailsModal from '../../components/request/UserDetailsModal'; // Import the new modal
...
export default function FindMedicinesPage() {
  ...
  const [isLoading, setIsLoading] = useState(false); // Default to false until user details are submitted
  ...
  const [userDetails, setUserDetails] = useState<{ name: string; contact: string } | null>(null);
  ...
  const fetchMedicines = useCallback(debounce(async (page: number, search: string, filter: string, sort: string) => {
    if (!hasInitiatedSearch) return; // Don't fetch until user has submitted details
    setIsLoading(true);
    try {
  ...
  }, 500), [slug, itemsPerPage, userLocation, hasInitiatedSearch]);

  // --- EFFECT for User Details Modal ---
  useEffect(() => {
    const storedUserDetails = sessionStorage.getItem('userDetails');
    const hasSkipped = sessionStorage.getItem('hasSkippedUserDetails') === 'true';
  ...
  }, [currentPage, searchQuery, filterBy, sortBy, fetchMedicines, hasInitiatedSearch]);
  ...
  const handleUserDetailsSubmit = (details: { name: string; contact: string }) => {
  ...
  const handleUserDetailsSkip = () => {
      sessionStorage.setItem('hasSkippedUserDetails', 'true');
      setIsUserDetailsModalOpen(false);
      setHasInitiatedSearch(true);
  };
