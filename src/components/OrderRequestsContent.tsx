...
const OrderRequestsContent: React.FC = () => {
  ...
  return (
    <Paper sx={{ p: { xs: 2, sm: 3 }, borderRadius: '28px', boxShadow: '0 8px 24px rgba(0,0,0,0.08)', overflow: 'hidden', minHeight: '400px' }}>
      {selectedRequestId ? (
            ...
            <List sx={{ p: 0 }}>
              {requests.map((request) => (
                <Paper key={request._id} component="li" sx={{ p: 2, mb: 2, borderRadius: '12px', listStyle: 'none', background: 'rgba(0, 0, 0, 0.04)' }} elevation={0}>
                  ...
                  <Box sx={{ mt: 2 }}>
                    <Button variant="outlined" size="small" onClick={() => handleManageRequest(request._id)}>Manage Request</Button>
                  </Box>
                </Paper>
              ))}
            </List>
