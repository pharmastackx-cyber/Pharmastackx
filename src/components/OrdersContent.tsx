        ...
        >
          {tabConfig.map((tab, index) => (
            <Tab
              ...
              sx={{ px: { xs: 0, sm: 2 }, minWidth: 'auto' }}
            />
          ))}
        </Tabs>
        <Box sx={{ px: { xs: 2, sm: 0 }, bgcolor: 'white', borderBottomLeftRadius: '20px', borderBottomRightRadius: '20px' }}>
          <TabPanel value={tabValue} index={0}>
            {processingOrders.length > 0 ?
                processingOrders.map((order) => <OrderCard key={order._id} order={order} />) :
                <Typography sx={{textAlign: 'center', py: 4, color: 'text.secondary'}}>No orders in process.</Typography>}
          </TabPanel>
          <TabPanel value={tabValue} index={1}>
            {completedOrders.length > 0 ?
                completedOrders.map((order) => <OrderCard key={order._id} order={order} />) :
                <Typography sx={{textAlign: 'center', py: 4, color: 'text.secondary'}}>No completed orders.</Typography>}
          </TabPanel>
          <TabPanel value={tabValue} index={2}>
            {cancelledOrders.length > 0 ?
                cancelledOrders.map((order) => <OrderCard key={order._id} order={order} />) :
                <Typography sx={{textAlign: 'center', py: 4, color: 'text.secondary'}}>No cancelled orders.</Typography>}
          </TabPanel>
        </Box>
    </Box>
  );
